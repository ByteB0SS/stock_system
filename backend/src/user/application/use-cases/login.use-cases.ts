...
import { TOKEN_SYMBOL, TokenPort } from "src/auth/application/ports/token.port"
import { UseCasePort } from "@shared/application/ports/use-case.port"
import { USER_REPOSITORY_SYMBOL, UserRepositoryPort } from "../ports/user-repository.port"
...
export class LoginUserUseCase implements UseCasePort<loginUserInputDto, Promise<loginRetrunType>> {
  constructor(
    ...
    @Inject(TOKEN_SYMBOL) private readonly tokenManager: TokenPort,
  ...
  async execute(credentials: loginUserInputDto, returnUser: boolean = true): Promise<loginRetrunType> {
    ...
    const passwordIsCorrect = await this.hasher.isValid(credentials.password, userWithSameEmail.getPrimitiveProps().password)
    if (!passwordIsCorrect) {
      throw new HttpException("senha ou email incorretos", 400)
    }

    const userProps = userWithSameEmail.getPrimitiveProps()
