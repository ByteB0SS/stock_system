...
export class PrismaUserRepositoryAdapter implements UserRepositoryPort {
  ...
  async create(user: User): Promise<User> {
    const { createdAt, updatedAt, deletedAt, ...dataToSave } = user.getPrimitiveProps()
  ...
  async findByEmail(email: EmailVO): Promise<User | null> {
    const user = await this.prisma.user.findFirst({
      where: { email: email.get(), status: "ACTIVE" },
    })
    if (!user) {
      return null
    }
  ...
  async findBySlug(slug: SlugVO): Promise<User | null> {
    const user = await this.prisma.user.findUnique({
      where: { slug: slug.get(), status: "ACTIVE" },
    })
    if (!user) return null
    return this.restoreUser(user)
  }
  ...
  async update(user: User): Promise<User> {
    const { updatedAt, createdAt, deletedAt, id, ...dataToUpdate } = user.getPrimitiveProps()
