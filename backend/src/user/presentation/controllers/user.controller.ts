...
import { AuthGuard } from "src/auth/presentation/guards/auth.guard"
...
export class UserController {
  ...
  @ApiConflictResponse({
    description: "Erro de conflito, email em uso",
    type: RegisterUserConflictResponseDto,
  })
  async registerUser(@Body() dto: RegisterUserDto) {
    const user = await this.userRegister.execute(dto)
    const userProps = user.getProps()
    console.log(userProps.password)
    const tokens = await this.login.execute({ email: user.getPrimitiveProps().email, password: dto.password }, false)
  ...
  async changePassword(@Body() dto: changeUserPassowrdDto, @Param("id") id: string) {
    const isUpdated = await this.userPasswordUpdater.execute({
      newPassowrd: dto.newPassword,
  ...
  async getUsers(@Query("limit") limit?: number, @Query("last_id") last_id?: string, @Query("slug_filter") slug_filter?: string, @Query("name_filter") name_filter?: string) {
    const users = await this.usersGetter.execute({
      lastId: last_id,
      limit: limit,
      slugFilter: slug_filter,
