...
@Injectable()
export class AccessAuthorizerUseCase implements UseCasePort<AccessAuthorizerInput, AccessAuthorizerOutput> {
  constructor(
  ...
  async execute(input: AccessAuthorizerInput): Promise<AccessAuthorizerOutput> {
    const result = this.tokenPort.isValid(input.token, input.tokenType ?? "ACCESS_TOKEN")
